(ns malli-env.core-test
  (:require [clojure.test :refer [deftest is deftest]]
            [malli-env.core :refer [->kebab env env*]]))

(deftest ->kebab-test (is (= :asdf-1234 (->kebab "ASDF_1234"))))

(deftest simple-test-test
  (is (= [:valid {:asdf-zxcv "zxcv"}]
         (env* [:map [:asdf-zxcv :string]] {"ASDF_ZXCV" "zxcv"}))))

(deftest keyword-test
  (is (= [:valid {:hello :one}]
         (env* [:map [:hello [:and :keyword [:enum :one :two]]]]
               {"HELLO" "one"}))))

(deftest removes-unrecognized-keys-test
  (is (= [:valid {}]
         (env* [:map [:asdf {:optional true} :string]] {"HHHH" "000"}))))

(deftest decode-ints-test
  (is (= [:valid {:asdf 1234}] (env* [:map [:asdf :int]] {"ASDF" "1234"}))))

(deftest decode-booleans-test
  (is (= [:valid {:asdf true}] (env* [:map [:asdf :boolean]] {"ASDF" "true"})))
  (is (= [:valid {:asdf true}] (env* [:map [:asdf :boolean]] {"ASDF" "TrUE"})))
  (is (= [:valid {:asdf false}]
         (env* [:map [:asdf :boolean]] {"ASDF" "FaLSe"}))))

(deftest defaults-test
  (is (= [:valid {:asdf 555}] (env* [:map [:asdf {:default 555} :int]] {}))))

(deftest optional-test
  (is (= [:valid {}] (env* [:map [:asdf {:optional true} :int]] {}))))
...
(deftest regex-test
  (let [[result message] (env* [:map [:asdf #"A"] [:zxcv #"B"]]
                               {"ASDF" "C", "ZXCV" "~~B~~"})]
    (is (= :invalid result))
...
(deftest boolean-test
  (let [[result message]
          (env* [:map [:flag1 :boolean] [:flag2 :boolean] [:flag3 :boolean]
...
(deftest valid-env-test
  (let [exit-code (atom nil)]
    (is (= ""
           (with-out-str (env [:map [:param-1 :string]]
                              {"PARAM_1" "value1"}
                              #(reset! exit-code %)))))
    (is (nil? @exit-code))))
...
(deftest invalid-env-test
  (let [exit-code (atom nil)]
    (is (re-find #":param1.+missing required key"
                 (with-out-str
                   (env [:map [:param1 :string]] {} #(reset! exit-code %)))))
    (is (= 1 @exit-code))))
